...
\begin{frame}{审计结果}
    ...
    \begin{center}
        \Large
        三层排序相关 $\rho \approx \textbf{0.62}$，但 $n=10$ \alert{CI 宽}
    \end{center}
    \vspace{0.2em}
    \begin{center}
        \normalsize
        L1 / L3 下界跨 0 $\Rightarrow$ \textbf{排序级粗筛信号}，需 $n=100$ 收紧。
    \end{center}

    \vspace{0.3em}
    \footnotesize\centering\itshape
    Bootstrap percentile over 10 LOO pairs；正是阶段 2 扩 unlearn 的动机。
\end{frame}
