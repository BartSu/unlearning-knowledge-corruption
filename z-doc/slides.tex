...
\begin{frame}{三层是什么：一张同心圆图}
    ...
    \begin{tikzpicture}[font=\small]
        ...
        \fill[mLightBrown!30]  (0,0) circle (2.0) (0,0) circle (1.0);
        \fill[mDarkTeal!10]    (0,0) circle (3.2) (0,0) circle (2.0);
        ...
        \node[right=4mm, align=left] at (1.0, 0.0)
            {\textbf{L1}：\emph{想遗忘的}\\ \footnotesize = 被 unlearn 那批样本};
        \node[right=4mm, align=left] at (2.0, 1.5)
            {\textbf{L2}：\emph{连带忘的}\\ \footnotesize = 同 cluster · 没训过};
        \node[right=4mm, align=left] at (3.2, 2.6)
            {\textbf{L3}：\emph{不该忘却被波及}\\ \footnotesize = 别的 cluster};
    \end{tikzpicture}

    \vspace{0.4em}
    \centering
    \footnotesize 越往外离 forget set 越远 $\Rightarrow$ 副作用应越小
\end{frame}
