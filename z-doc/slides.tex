...
\begin{frame}{L1 / L2 / L3 在 10×10 矩阵里的位置}
    ...
    \begin{tikzpicture}[font=\footnotesize]
        ...
        \node[rotate=90, above] at (-0.15, -10*\s/2) {unlearn 簇 $m$ →};

        \draw[-Latex, thick] (5*\s+2.2, 0.3) -- node[above, sloped]
            {\textbf{对角线 $m=e$}} (3.5*\s, -3.5*\s+\s*0.5);
        \node[right, align=left] at (5*\s+2.2, 0.3)
            {\textbf{L1} = train 分片\\ \textbf{L2} = val / test 分片};

        \draw[-Latex, thick] (5*\s+2.2, -1.5) -- node[above, sloped]
            {\textbf{非对角线 $m\neq e$}} (6.5*\s, -3.5*\s+\s*0.5);
        \node[right] at (5*\s+2.2, -1.5)
            {\textbf{L3} = test 分片（别的簇）};
    \end{tikzpicture}
\end{frame}
