...
\caption{\textbf{Two-stage framework.}
\textbf{(1)} \emph{Measure three-layer unlearning impact}: each candidate $\Df$ is unlearned and the resulting checkpoint is evaluated by base-normalised PPL on three semantic layers --- $\Lone$ forget, $\Ltwo$ same-topic neighbours, $\Lthree$ off-target bystanders --- yielding a layer score per layer (geo-mean of $r$). Impact decays with semantic distance ($\Lone{>}\Ltwo{>}\Lthree$).
\textbf{(2)} \emph{Audit risk before unlearning}: a forget-set geometry descriptor $\phi(\Df)$ is fed to a cheap predictor that outputs the predicted three-layer profile, enabling pre-screening of candidate forget sets without running unlearning.}
\label{fig:hero}
\end{figure}
...
% method and insight
We address this question in two stages (Fig.~\ref{fig:hero}): measuring three-layer unlearning impact under a controlled cross-evaluation matrix, then auditing the predicted impact profile from forget-set geometry alone. We cluster WikiText-103 passages into semantic
...
\scriptsize
\put(0,0){\framebox(76,34){\shortstack{Forget set\\$\Lone$}}}
\put(82,0){\framebox(76,34){\shortstack{Neighbours\\$\Ltwo$}}}
\put(164,0){\framebox(76,34){\shortstack{Bystanders\\$\Lthree$}}}
\end{picture}}
\vspace{0.35em}
\refstepcounter{figure}\label{fig:triplet-schema}
\parbox{0.96\columnwidth}{\small\textbf{Figure~\thefigure:} Triplet schema. WikiText-103 passages are filtered, embedded, and clustered; each non-noise semantic cluster yields $10$ triplets. Each triplet's three disjoint $50$-text splits instantiate the three layers: the train split is the forget set ($\Lone$), the val/test splits are same-topic neighbours ($\Ltwo$), and test splits from triplets in other clusters serve as cross-topic bystanders ($\Lthree$).}
\end{center}
