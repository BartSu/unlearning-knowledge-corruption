...
\twocolumn[
  ...
  \begin{@twocolumnfalse}
    \begin{abstract}
      Large language model (LLM) unlearning aims to remove target knowledge represented by a forget set $\Df$ through post-training updates, but its effect is not captured by a single forget/retain score. We propose evaluating unlearning as a \emph{three-layer impact profile}: intended degradation on the forget set, neighbour impact on held-out same-topic samples, and bystander impact on cross-topic samples. We denote these layers by $\Lforget$, $\Lneigh$, and $\Lbyst$. Using WikiText-103 triplets, $100$ GradAscent unlearning runs on Llama-3.1-8B-Instruct, and a $100\times100$ cross-PPL evaluation matrix, we measure $1.5$M per-sample perplexity ratios. The impact decays strictly across layers ($\Lforget{=}1.96\times$, $\Lneigh{=}1.32\times$, $\Lbyst{=}1.19\times$), but bystander impact remains measurable: $73.8\%$ of cross-topic samples have elevated PPL, and the profile varies substantially with the chosen forget set. We then formulate \emph{forget-set audit}: predicting the three-layer impact profile from forget-set geometry alone. A $12$-feature Random Forest auditor gives rank-significant predictions for all layers, with Spearman $\rho{=}0.56/0.87/0.59$ for $\Lforget/\Lneigh/\Lbyst$, enabling a cheap pre-screen before full unlearning evaluation. Replicating the full pipeline with NPO (a separate unlearning algorithm) yields independently rank-significant predictions ($\rho{=}0.66/0.82/0.51$, all CIs above zero), confirming the audit is unlearner-agnostic.
    \end{abstract}
    \vspace{0.8em}
  \end{@twocolumnfalse}
]
...
Large language models increasingly need mechanisms for removing target
knowledge after deployment. Copyright disputes, privacy regulation, user
deletion requests, and the discovery of toxic, private, or copyrighted content
in pretraining corpora all create cases where a model should stop reproducing
or relying on specific knowledge represented by a forget set $\Df$
\citep{maini2024tofu, shi2024muse, dorna2025openunlearning}. Since retraining
from scratch is usually impractical, \emph{LLM unlearning} uses post-training
updates to suppress this target knowledge while preserving the rest of the
model. Modern benchmarks such as TOFU and MUSE therefore evaluate both sides of
the requirement: forget the target knowledge, and preserve general utility.
...
unlearning impact remains unclear. We do not yet have a direct measurement
...
unclear whether collateral damage decays cleanly with semantic distance or remains
diffuse across unrelated content. Second, most benchmarks fix the forget split and compare unlearning algorithms.
...
each candidate is expensive. We therefore ask a more data-first question: can we
estimate which forget sets are likely to be damaging \emph{before} running
unlearning?

We study unlearning impact through a controlled WikiText-103 triplet
...
This matrix lets us define and measure a \emph{three-layer impact profile}.
For an unlearned checkpoint $m$ and evaluation triplet $e$, $\Lone$ is
\textbf{intended degradation}: the diagonal train split ($m{=}e$), i.e., the
requested deletion target itself. $\Ltwo$ is \textbf{locality damage}: the
diagonal validation/test splits ($m{=}e$), i.e., unseen same-cluster neighbours
that should ideally be preserved. $\Lthree$ is \textbf{spillover damage}:
off-diagonal test splits ($m{\neq}e$), i.e., unrelated-topic bystanders. Across
$1.5$M evaluations, impact decays strictly
...
related statistics). Under leave-one-out cross-validation, a Random Forest
regressor predicts the three-layer impact profile from these features
alone. The resulting rankings are significant for all layers, with Spearman
correlations of $0.56$, $0.87$, and $0.59$ for $\Lone$, $\Ltwo$, and $\Lthree$;
...
\begin{enumerate}
    ...
    \item We introduce a measurement framework for three-layer unlearning
    impact, separating intended forget-set degradation, same-topic locality
    damage, and cross-topic spillover damage in a single cross-PPL matrix.
    \item We provide quantitative evidence that unlearning impact decays
    with semantic distance but collateral damage remains non-zero far from the forget set:
    ...
    impact profile from forget-set geometry alone. A $12$-feature Random
    Forest auditor achieves rank-significant predictions for all three
    layers, including $\rho{=}0.87$ and $R^2{=}0.74$ for locality damage,
    enabling cheap screening before full unlearning.
\end{enumerate}
...
Let $f$ be the base model and let $\mathcal{U}$ be a fixed post-training
unlearning procedure. A forget set $\Df$ operationally represents the target
knowledge to be removed, and applying unlearning yields
\begin{equation}
    f_{\Df}' \;=\; \mathcal{U}(f, \Df).
\end{equation}
Our goal is not to construct adversarial or corrupted evaluation data. Instead,
we ask how the impact of this update spreads from the intended target to
nearby and unrelated knowledge, and whether that impact can be predicted before
running $\mathcal{U}$.

\paragraph{Three-layer evaluation sets.}
For each candidate $\Df$, we construct two disjoint evaluation sets:
$\mathcal{D}_{N}$, held-out same-topic neighbours of $\Df$, and
$\mathcal{D}_{B}$, cross-topic bystanders. These sets satisfy
\begin{align}
    \Df \cap \mathcal{D}_{N} &= \emptyset,\\
    \Df \cap \mathcal{D}_{B} &= \emptyset,\\
    \mathcal{D}_{N} \cap \mathcal{D}_{B} &= \emptyset,\\
    \mathrm{sim}(\mathcal{D}_{N}, \Df) &\ge \gamma,\\
    \mathrm{sim}(\mathcal{D}_{B}, \Df) &\le \delta,\\
    \max_{S\in\{\Df,\mathcal{D}_{N},\mathcal{D}_{B}\}}\PPL_f(S) &\le \tau,\\
    \min_{S\in\{\Df,\mathcal{D}_{N},\mathcal{D}_{B}\}}|S| &\ge k.
\end{align}
Here $\mathrm{sim}(\cdot,\cdot)$ measures semantic relatedness between sets,
$\gamma$ enforces same-topic neighbourhood for $\mathcal{D}_{N}$, $\delta$
enforces cross-topic separation for $\mathcal{D}_{B}$, $\tau$ keeps the
evaluation text within the base model's linguistic support, and $k$ controls
coverage.

\paragraph{Three-layer impact profile.}
For a sample $x$, we measure post-unlearning impact by the base-normalised PPL
ratio
\begin{equation}
    r_{\Df}(x) \;=\;
    \frac{\PPL_{f_{\Df}'}(x)}{\PPL_f(x)}.
\end{equation}
The impact profile of $\Df$ is the vector
\begin{equation}
    I(\Df)
    =
    \left(
...
where
\begin{align}
    \Lforget(\Df) &= \geomean_{x\in\Df}\, r_{\Df}(x),\\
    \Lneigh(\Df) &= \geomean_{x\in\mathcal{D}_{N}}\, r_{\Df}(x),\\
    \Lbyst(\Df) &= \geomean_{x\in\mathcal{D}_{B}}\, r_{\Df}(x).
\end{align}
These are the intended degradation, neighbour impact, and bystander impact,
respectively. High $\Lforget$ is the intended forgetting signal; high
$\Lneigh$ or $\Lbyst$ is collateral damage.

\paragraph{Forget-set audit.}
Measuring $I(\Df)$ requires running $\mathcal{U}$ and evaluating $f_{\Df}'$.
The audit problem forbids this at prediction time: given only features
$\phi(\Df)$ of the candidate forget set, learn a predictor
\begin{equation}
    h(\phi(\Df)) \;\approx\; I(\Df),
\end{equation}
with no access to $f_{\Df}'$, $\mathcal{D}_{N}$, or $\mathcal{D}_{B}$ except
when producing held-out evaluation labels. We evaluate $h$ primarily as a
ranking tool over candidate forget sets, so the practical objective is to rank
candidates by predicted impact profile before spending GPU time on full
unlearning and cross-evaluation.
