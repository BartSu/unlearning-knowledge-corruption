...
\begin{tabular}{@{}lll@{}}
    ...
    \midrule
    \texttt{train}      & forget set $\Df$              & unlearning target \\
    \texttt{validation} & neighbours                    & retain/probe \\
    \texttt{test}       & probe texts                   & diag/off-diag probe \\
    \bottomrule
\end{tabular}
\end{center}
The within-cluster but mutually disjoint design supports both same-topic and
cross-topic evaluation by re-indexing the same triplet collection. Different
triplets may overlap across clusters because sampling is independent, but
within a triplet the three splits never share a text.
...
Following Sec.~\ref{sec:problem}, we evaluate every unlearned checkpoint on
every triplet split and normalise each PPL by the corresponding base-model
PPL. With $100$ checkpoints, $100$ evaluation triplets, $3$ splits, and $50$
texts per split, the sweep yields $1.5$M per-sample ratios. The diagonal
entries provide $5{,}000$ forget-set samples and $10{,}000$ same-topic held-out
samples; off-diagonal test splits provide $495{,}000$ cross-topic bystander
samples.

For reporting, we aggregate the layer labels defined in Sec.~\ref{sec:problem}
by geometric mean and by the fraction of samples with ratio $r{>}1.1$. The
full cross-PPL sweep takes $\approx 6$\,h\,$11$\,min on one H100
($\approx 2.2$\,s per checkpoint--triplet pair). We cache a $1.5$M-row
long-format table for downstream auditing and sanity checks.
...
PPL ratio. We use audit primarily for \emph{ranking}: identifying which
candidate forget sets are likely to produce larger intended degradation,
same-topic locality damage, or cross-topic spillover.
...
\begin{tabular}{@{}llccc@{}}
    \toprule
    Metric & Alg. & $\Lforget$ & $\Lneigh$ & $\Lbyst$ \\
    \midrule
    geo $r$
...
LLM unlearning is not fully described by whether the requested forget set is
removed. The same update can damage held-out same-topic knowledge and
unrelated-topic bystanders, so we formalised unlearning impact as a
three-layer profile: intended forget-set degradation ($\Lone$), same-topic
locality damage ($\Ltwo$), and cross-topic spillover damage ($\Lthree$). Using
WikiText-103 triplets, $100$ candidate forget sets, Llama-3.1-8B-Instruct, and
a fixed GradAscent unlearner, we built a $1.5$M-sample cross-PPL matrix and
then asked whether this profile can be predicted from forget-set geometry
alone.

The results show that impact is structured but not local. Damage decays across
the three layers, with geo-mean ratios of $1.96\times$, $1.32\times$, and
$1.19\times$, yet spillover remains measurable: $73.8\%$ of cross-topic
samples have elevated PPL. The forget set itself also matters, with $\Lone$
varying by $1.59\times$ under fixed algorithm and hyperparameters. A
$12$-feature Random Forest auditor predicts all three layers with
rank-significant leave-one-out correlations, including $\rho{=}0.87$ and
$R^2{=}0.74$ for locality damage; the same audit remains rank-significant on
NPO.

These findings support a \emph{screen-then-run} workflow: cheaply rank
candidate forget sets by predicted risk, then run full unlearning and
cross-evaluation on the candidates that matter most. The audit does not replace
the final evaluation, but it makes forget-set selection less blind and less
expensive. Future work should scale across models and unlearners, add QA-style
utility metrics, enrich the geometry descriptor, and construct controlled
forget sets to test which properties causally drive locality and spillover
damage.
...
is $0.71$ (storm). The full table is included in the released summary JSON.
