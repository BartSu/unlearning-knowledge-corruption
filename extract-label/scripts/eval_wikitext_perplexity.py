...
Output format is JSON only (no CSV):
  - baseline JSON: base model loss/ppl on each triplet's train/validation/test
  - metrics JSON: cross-eval rows for each (unlearn model triplet, eval triplet)
    pair, with base vs unlearn loss/ppl on train/validation/test
...
WIKITEXT_DIR_CANDIDATES = (
    Path(__file__).resolve().parent.parent / "data-preparation" / "data" / "wikitext_hdbscan_triplets",
    Path(__file__).resolve().parent.parent / "data-preparation" / "data" / "wikitext",
)
...
def load_triplet_texts(triplet_dir):
    return {
        split_name: load_texts(resolve_triplet_split_path(triplet_dir, split_name))
        for split_name in TRIPLET_SPLITS
    }
...
def has_complete_triplet_metrics(metrics):
    normalized = normalize_triplet_metrics(metrics)
    return all(isinstance(normalized.get(split_name), dict) for split_name in TRIPLET_SPLITS)
...
def compute_baselines(args):
    """Base model loss/ppl on each triplet's train/validation/test sets."""
    ...
    if args.resume and baseline_path.exists():
        ...
        n_cached = sum(
            ...
            if has_complete_triplet_metrics(metrics)
        )
    ...
    if args.test_path and "shared_test" not in baseline:
        test_texts = load_texts(args.test_path)
    ...
    remaining = [
        ...
        if not has_complete_triplet_metrics(baseline["triplets"].get(d.name))
    ]
    ...
    for i, tdir in enumerate(remaining, 1):
        name = tdir.name
        texts_by_split = load_triplet_texts(tdir)
        ...
        print(f"  [{i:3d}/{len(remaining)}] {name}: "
              f"train={baseline['triplets'][name]['train']['loss']:.4f}  "
              f"validation={baseline['triplets'][name]['validation']['loss']:.4f}  "
              f"test={baseline['triplets'][name]['test']['loss']:.4f}")
...
def compute_labels(args):
    ...
    missing_baselines = [
        ...
        if not has_complete_triplet_metrics(baseline.get("triplets", {}).get(tdir.name))
    ]
    ...
    eval_triplet_texts = {
        tdir.name: load_triplet_texts(tdir)
        for tdir in eval_triplet_dirs
    }
            ...
            if (
                ...
                done_pairs = {
                    ...
                    if (
                        ...
                        and has_complete_triplet_metrics(row.get("base"))
                        and has_complete_triplet_metrics(row.get("unlearn"))
                    )
                }
    ...
    shared_test_texts = load_texts(args.test_path) if args.test_path else None
    ...
    for mdir in model_dirs:
        ...
        for eval_triplet in eval_triplet_names:
            ...
            if not has_complete_triplet_metrics(triplet_base):
