...
Pipeline:
  ...
  2. Compute cross-eval metrics:
     python eval_wikitext_perplexity.py --saves_dir ../data-preparation/unlearn/saves/wikitext_unlearn --end 50 --resume

  3. With QA data (qa_prompt perplexity):
     python eval_wikitext_perplexity.py --baseline --data_dir ../data-preparation/data/wikitext_hdbscan_triplets_qa --text_field qa_prompt --resume
"""
...
WIKITEXT_DIR_CANDIDATES = (
    PROJECT_ROOT / "data-preparation" / "data" / "wikitext_hdbscan_triplets",
    PROJECT_ROOT / "data-preparation" / "data" / "wikitext_hdbscan_triplets_qa",
    PROJECT_ROOT / "data-preparation" / "data" / "wikitext",
)
OUT_DIR = Path(__file__).resolve().parent
DEFAULT_BASE_MODEL = "meta-llama/Llama-3.1-8B-Instruct"
BASELINE_FILE = "wikitext_baseline.json"
BASELINE_DETAIL_FILE = "wikitext_baseline_detail.json"
DEFAULT_METRICS_FILE = "wikitext_cross_metrics.json"
DEFAULT_DETAIL_FILE = "wikitext_cross_metrics_detail.json"
...
def compute_avg_loss(model, tokenizer, texts, max_length=512, batch_size=4):
    """Per-sample and aggregate per-token cross-entropy loss and perplexity.

    Returns (avg_loss, avg_ppl, per_sample) where per_sample is a list of
    {"loss": float, "ppl": float, "n_tokens": int} dicts, one per text.
    """
    model.eval()
    ...
    for i in range(0, len(texts), batch_size):
        ...
        outputs = model(input_ids=input_ids, attention_mask=encodings["attention_mask"])
        ...
        sample_tokens = mask.sum(dim=1)
        sample_loss = (per_token * mask).sum(dim=1) / sample_tokens.clamp(min=1)

        for j in range(len(batch_texts)):
            sl = float(sample_loss[j])
            st = int(sample_tokens[j])
            per_sample.append({
                "loss": round(sl, 6),
                "ppl": round(float(np.exp(sl)), 2),
                "n_tokens": st,
            })
            total_loss += sl * st
            total_tokens += st
    ...
    return avg_loss, float(np.exp(avg_loss)), per_sample
...
def _detail_path(summary_path: Path) -> Path:
    """Derive the detail file path from the summary file path.
    e.g. foo.json -> foo_detail.json"""
    return summary_path.with_name(
        summary_path.stem + "_detail" + summary_path.suffix
    )
...
def evaluate_triplet_splits(model, tokenizer, texts_by_split, max_length, batch_size):
    """Returns (summary, detail) where summary has aggregate loss/ppl and
    detail has per-sample lists, both keyed by split name."""
    summary, detail = {}, {}
    for split_name, texts in texts_by_split.items():
        avg_loss, avg_ppl, per_sample = compute_avg_loss(
            model, tokenizer, texts, max_length=max_length, batch_size=batch_size
        )
        summary[split_name] = metric_dict(avg_loss, avg_ppl)
        detail[split_name] = per_sample
...
def compute_baselines(args):
    ...
    detail_path = _detail_path(baseline_path)

    baseline = {}
    baseline_detail = {"triplets": {}}
    ...
    baseline_detail.setdefault("triplets", {})
    ...
    for i, tdir in enumerate(remaining, 1):
        ...
        summary, detail = evaluate_triplet_splits(
            model,
            tokenizer,
            texts_by_split,
        ...
        baseline["triplets"][name] = summary
        baseline_detail["triplets"][name] = detail
        split_parts = "  ".join(
            f"{s}={summary[s]['ppl']:.2f}" for s in summary
        )
    ...
    print(f"Baseline detail  saved to {detail_path}")
...
def compute_labels(args):
    ...
    detail_out_path = _detail_path(output_path)
    ...
    baseline_detail_path = _detail_path(baseline_path)
    baseline_detail = {"triplets": {}}
    if baseline_detail_path.exists():
    ...
    if (saves / "config.json").exists():
        ...
        if not model_dirs:
            # "triplet" contains "triple" so *triple* re-matches every *triplet* dir;
            # dedup via set union before sorting.
            model_dirs = sorted(set(saves.glob("*triplet*")) | set(saves.glob("*triple*")))
    ...
    metrics = {**_header, "results": []}
    metrics_detail = {**_header, "results": []}
        ...
        try:
            ...
            if (
                ...
                if detail_out_path.exists():
                    with open(detail_out_path) as f:
    ...
    metrics.update(_header)
    metrics_detail.update(_header)
    ...
    for mdir in model_dirs:
        ...
        shared_test_summary, shared_test_detail = None, None
        if shared_test_texts is not None and isinstance(baseline.get("shared_test"), dict):
            shared_test_loss, shared_test_ppl, shared_test_ps = compute_avg_loss(
                model, tokenizer, shared_test_texts, args.max_length, args.batch_size,
            )
            shared_test_summary = metric_dict(shared_test_loss, shared_test_ppl)
            shared_test_detail = shared_test_ps
        ...
        for eval_triplet in eval_triplet_names:
            ...
            unlearn_summary, unlearn_detail = evaluate_triplet_splits(
                model, tokenizer, eval_triplet_texts[eval_triplet],
            ...
            base_detail = {
                split_name: baseline_detail.get("triplets", {}).get(eval_triplet, {}).get(split_name)
                for split_name in eval_splits
                if split_name in triplet_base
            }
            if shared_test_summary is not None:
                base_summary["shared_test"] = baseline["shared_test"]
                unlearn_summary["shared_test"] = shared_test_summary
                base_detail["shared_test"] = baseline_detail.get("shared_test")
                unlearn_detail["shared_test"] = shared_test_detail
            ...
            metrics["results"].append({
                **row_id,
                "base": base_summary,
                "unlearn": unlearn_summary,
            })
            metrics_detail["results"].append({
                **row_id,
                "base": base_detail,
                "unlearn": unlearn_detail,
            })
        ...
        _save_json(metrics_detail, detail_out_path)
    ...
    if metrics["results"]:
        ...
        print(f"\nSummary saved to {output_path}  ({len(metrics['results'])} rows)")
        print(f"Detail  saved to {detail_out_path}")
        print(
