...
def part2_forget_features(clusters: list[str]) -> pd.DataFrame:
    """Read 12-dim forget-set intrinsic geometry from stage ④ CSV.

    Extraction logic lives in
      4.feature-engineering/scripts/extract_forget_geometry.py
    so this stage only consumes the feature store; it does not embed.
    """
    if not FORGET_GEOMETRY_CSV.exists():
        raise FileNotFoundError(
            f"{FORGET_GEOMETRY_CSV} not found — run "
            f"`python 4.feature-engineering/scripts/extract_forget_geometry.py "
            f"--triplets '{' '.join(clusters)}'` first."
        )
    df = pd.read_csv(FORGET_GEOMETRY_CSV)
    missing = [c for c in clusters if c not in set(df["forget_cluster"])]
    if missing:
        raise ValueError(
            f"Forget clusters missing in {FORGET_GEOMETRY_CSV.name}: {missing}. "
            f"Re-run extract_forget_geometry.py with --triplets covering them."
        )
    return df[df["forget_cluster"].isin(clusters)].sort_values("forget_cluster").reset_index(drop=True)
