"""Held-out (LOO) R² / MAE / RMSE on the audit predictor.

Each fold: RandomForestRegressor (n_estimators=200, min_samples_leaf=2,
random_state=0) trained on n-1 forget sets, predicts the geo_L{1,2,3}
ratio of the 1 held-out forget set.
...
def main():
    ...
    summary["heldout_r2_mae"] = {
        "n": int(len(df)),
        "protocol": "LOO over forget sets; RandomForest(n=200, min_leaf=2, seed=0) on 12-d forget-set geometry",
        "note": "Baseline predicts the mean of the n-1 training targets (also LOO).",
        "layers": out,
    }
